...
export default function CameraModal() {
  const { camOpen, videoRef, closeCamera, capturePhoto, torchOn, torchSupported, torchError, flashChecked, toggleTorch } =
    useSplit();
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col">
      ...
      <div className="absolute bottom-0 inset-x-0 pb-10 pt-8 flex flex-col items-center bg-gradient-to-t from-black to-transparent">
        ...
        {flashChecked && !torchSupported && (
          <p className="mb-4 mx-4 max-w-xs text-center bg-black bg-opacity-60 text-gray-200 text-xs rounded-xl px-3 py-2">
            💡 If it's too dark, turn on your phone's flashlight before scanning.
          </p>
        )}
        <button
